...
//         try {
//             const response = await makeApi(`/v1/get-all-staff`, "GET");
...
//             setAllAdmins(staffList);

//             // Set the salary for the selected staff if any staff is selected
//             if (selectedStaff) {
//                 const selectedStaffData = staffList.find(staff => staff.username === selectedStaff);
//                 if (selectedStaffData) {
//                     setSelectedStaffSalary(Number(selectedStaffData.basicsalary));
//                 }
//             }
//         } catch (error) {
//             console.error('Error fetching admins:', error);
...
//                 // Calculate the required data for the summary
//                 const salary = selectedStaffSalary || 0;
//                 const daysInMonth = new Date(currentYear, currentMonth, 0).getDate();
//                 const perDaySalary = salary / daysInMonth;
//                 const perMinuteSalary = Math.floor(perDaySalary / (9 * 60)); // Using Math.floor to ensure integer values
//                 const { totalHalfDays, totalFullDays, totalLeaves, totalShortLeaveMinutes, totalActiveMinutes } = response.data;
//                 const totalSalary = Math.floor(perMinuteSalary * totalActiveMinutes);
...
//         };
//         fetchMonthlyData();
//     }, [currentMonth, currentYear, selectedStaff, selectedStaffSalary]);
