...
function CampaignDetails() {
  ...
  const handleTabChange = (tab) => {
    ...
    switch (tab) {
      case 'New':
        setFilteredUsers(appliedUsers.filter(user => user.status === 'New'));
        break;
      case 'Rejected':
        setFilteredUsers(appliedUsers.filter(user => user.status === 'denied'));
        break;
      case 'approved':
        setFilteredUsers(appliedUsers.filter(user => user.status === 'approved'));
        break;
      default:
        setFilteredUsers(appliedUsers.filter(user => user.status === 'New'));
        break;
  ...
  const appliedUsers = [
    ...
    { id: 3, date: '2024-06-27', name: 'Alice Johnson', city: 'City D', contact: '1122334455', level: 'L3', image: "https://dwpdigital.blog.gov.uk/wp-content/uploads/sites/197/2017/07/jude-1-620x556.jpg", link: 'https://instagram.com/alicejohnson', content: 'photo', status: 'New', instagramFollowers: 3000, youtubeSubscribers: 3000, facebookFollowers: 300, engagementRate: 0.3 },
    { id: 1, date: '2024-06-25', name: 'John Doe', city: 'City B', contact: '1234567890', level: 'L1', image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTDJzEaxLN-jGRYYUO65pWu7Q9GXoNt4LUSSA&s", link: 'https://instagram.com/johndoe', content: 'photo', status: 'approved', providedContent: "https://kadence.com/wp-content/uploads/2023/02/Gymshark.png", instagramFollowers: 10000, youtubeSubscribers: 5000, facebookFollowers: 800, engagementRate: 0.7 },
    { id: 1, date: '2024-06-25', name: 'John Doe', city: 'City B', contact: '1234567890', level: 'L1', image: "https://images.pexels.com/photos/220453/pexels-photo-220453.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1", link: 'https://instagram.com/johndoe', content: 'photo', status: 'approved', providedContent: "", instagramFollowers: 10000, youtubeSubscribers: 5000, facebookFollowers: 800, engagementRate: 0.7 },
  ];
                  ...
                  <div key={user.id} className={`applied-user-card-unique ${user.status === 'accepted' ? 'accepted' : user.status === 'denied' ? 'denied' : ''}`}>
                    <div className="applied-user-content-unique">
                      ...
                      <div className="user-details-unique">
                        ...
                        {user.status === 'denied' && <p className='text-danger' > This user has been rejected </p>}
                        {user.status === 'approved' &&
                          <div>
                            <p className='text-success' > This user has been approved </p>
                            <div>
                              {user.providedContent ? (
                                <>
                                  <div className='d-flex gap-5' >
                                    <div className='bg-warning w-25' >
                                      <img src={user.providedContent} alt="providedContet" className="w-100" />
                                    </div>
